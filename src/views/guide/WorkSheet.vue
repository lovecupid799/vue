<template>
    <h1 class="work-tit"><span class="point">퍼블</span> 현황판</h1>
    <div class="work-head">
        <div class="work-anchor">
            <div class="left">
                <img
                class="work-space"
                src="data:image/png;base64,"
                alt="SPACE" />
                
                <a
                class="btn-txt sm primary"
                target="_blank"
                href="#"
                >퍼블 가이드<span class="ico-outlink_blue_b"></span
                ></a>
                
                <p class="work-txt"><strong>퍼블 파일</strong> : 파일 링크(화면아이디로 파일명 정의), <strong>TYPE</strong> : 파일 타입(T : Tab, P : Popup, M : Mail), <strong>처리상태</strong> : 작업 상태</p>
            </div>
            <div class="right">
                <template
                v-for="(menu, index) in anchorList"
                :key="index">
                <button
                    type="button"
                    class="btn-anchor"
                    :class="{ active: menuSelectedIndex === index }"
                    @click="onAnchorClick(event, menu, index)">
                    {{ menu.name }}
                </button>
                </template>
            </div>
        </div>
        <div class="work-table">
            <table>
                <colgroup>
                    <col style="width: 60px" />
                    <col style="width: 120px" />
                    <col style="width: 170px" />
                    <col style="width: 180px" />
                    <col style="width: 170px" />
                    <col style="width: 170px" />
                    <col style="width: 80px" />
                    <col style="width: 260px" />
                    <col style="width: 110px" />
                    <col style="width: 80px" />
                    <col style="width: 80px" />
                    <col style="width: auto" />
                </colgroup>
                <thead>
                <tr>
                    <th scope="col">No</th>
                    <th scope="col">1Depth</th>
                    <th scope="col">2Depth</th>
                    <th scope="col">3Depth</th>
                    <th scope="col">4Depth</th>
                    <th scope="col">5Depth</th>
                    <th scope="col">Type</th>
                    <th scope="col">퍼블 파일</th>
                    <th scope="col">작업자</th>
                    <th scope="col">완료 예정일</th>
                    <th scope="col">처리상태</th>
                    <th scope="col">비고</th>
                </tr>
                </thead>
            </table>
        </div>
    </div>
    <div className="workBody">
        <div className="workTable">
            <table>
                <colgroup>
                    <col style="width: 60px" />
                    <col style="width: 120px" />
                    <col style="width: 170px" />
                    <col style="width: 180px" />
                    <col style="width: 170px" />
                    <col style="width: 170px" />
                    <col style="width: 80px" />
                    <col style="width: 260px" />
                    <col style="width: 110px" />
                    <col style="width: 80px" />
                    <col style="width: 80px" />
                    <col style="width: auto" />
                </colgroup>
                <tbody>
                    <tr id="COM">
                        <td></td>
                        <td>레이아웃</td>
                        <td>메인 레이아웃</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td>
                            <!-- <router-link
                                target="_blank"
                                :to="{ name: 'SampleMain' }"
                                >샘플</router-link
                            > -->
                        </td>
                        <td>홍길동</td>
                        <td>24.00.00</td>
                        <td data-status=""></td>
                        <td>
                            <p class="etcTxt">
                                <span class="date">24.00.00</span>수정사항입력
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr class="hr" id="Menu1">
                        <td></td>
                        <td>Menu1</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr class="hr" id="Menu2">
                        <td></td>
                        <td>Menu2</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr class="hr" id="Menu3">
                        <td></td>
                        <td>Menu3</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr class="hr" id="Menu4">
                        <td></td>
                        <td>Menu4</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td><td></td>
                        <td data-status=""></td>
                        <td></td>
                        <td></td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</template>


<script setup>
import { ref, onMounted} from 'vue';

const anchorList = ref([
  { id: 'COM', name: 'Common' },
  { id: 'Menu1', name: 'Menu1' },
  { id: 'Menu2', name: 'Menu2' },
  { id: 'Menu3', name: 'Menu3' },
  { id: 'Menu4', name: 'Menu4' },
]);

const menuSelectedIndex = ref(null);

const onAnchorClick = (event, menu, index) => {
  const $target = `#${menu.id}`;
  const _offset = document.querySelector($target).offsetTop + 102;
  window.scrollTo({ top: _offset, left: 0 });
  menuSelectedIndex.value = index;
};

let $anchor = [];


onMounted(() => {
    
anchorList.value.forEach((e, idx) => {
    $anchor[idx] = document.querySelector(`#${e.id}`).offsetTop + 100;
});

const scrollAct = () => {
const isScrollTop = window.scrollY;

    $anchor.forEach((e, idx) => {
        if ($anchor[idx] < isScrollTop) {
        menuSelectedIndex.value = idx;
        } else if (isScrollTop < 100) {
        menuSelectedIndex.value = null;
        }
    });
};
  window.addEventListener('scroll', scrollAct);
});

// onBeforeUnmount(() => {
//   window.removeEventListener('scroll', scrollAct);
// });
</script>

<style lang="scss">
  @use "@/assets/scss/guide/guide";
</style>
  
